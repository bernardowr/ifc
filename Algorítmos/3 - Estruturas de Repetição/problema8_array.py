...
for i in range(n_consumidores):
    ...
    nome_consumidor = input(f"Insira o nome do consumidor {i+1}: ")
    ...
    tipos_consumidores.append(tipo_consumidor)
    nomes_consumidores.append(nome_consumidor)

    total_pagar = consumo * preco_kwh
    print(f"O total a pagar do consumidor {i+1} é de: R$ {total_pagar:.2f}")
...
print("Os consumidores e seus respectivos consumos foram:")
for i in range(n_consumidores):
    print(f"Consumidor {i+1}: {nomes_consumidores[i]}, consumo: {consumos[i]} kWh, tipo: {tipos_consumidores[i]}")